...
public:
	...
	static const std::map<unsigned long long, const char*>* GetMap() { return &CWeaponCapture::HashToName; }
	const std::vector<CPlayerWeapon>* GetCaptured() const { return &this->m_CapturedWeapons; }
	static const std::vector<const char*>* GetWeaponComponents() { return &CWeaponCapture::WeaponComponents; }
	~CWeaponCapture() {
		this->Delete();
	}
private:
	...
	inline static const std::map<unsigned long long, const char*> const HashToName {
	...
	//Now I would Hash these but theres no reason to because you would need the names too and having a hash list of them all isn't really worth it. Rather just hash at runtime. 
	inline static const std::vector<const char*> const WeaponComponents{
